...
class KubernetesSandboxSettings(BaseSandboxSettings):
    ...
    require_service_account: bool = Field(
        default=True,
        description="Whether a dedicated service account is required for "
        "sandbox pods. Keeping this enabled avoids accidental fallback to "
    ...
    automount_service_account_token: bool = Field(
        ...
        "pods. Disabled by default to reduce in-pod cluster API access.",
    )
    privileged: bool = Field(
        default=False,
        description="Whether sandbox session containers run in privileged mode.",
    )
    startup_timeout_seconds: PositiveInt = Field(
        default=120,
    ...
    api_request_timeout: Optional[PositiveInt] = Field(
        default=None,
        description="Timeout for Kubernetes API requests in seconds. If not "
        "set, client defaults are used.",
    )
